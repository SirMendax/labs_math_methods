...
def main():
    A = h.Helper.getMatrix('matrix.dat')
    B = h.Helper.getVector('vector.dat')

    floatA = np.array(h.Helper.getRealBandPositiveDefiniteMatrix(A), dtype = np.float32)
    floatB = np.array(B, dtype = np.float32)

    doubleA = np.array(h.Helper.getRealBandPositiveDefiniteMatrix(A), dtype = np.float64)
    doubleB = np.array(B, dtype = np.float64)
...
def printDesicion(A, B, P):
    for i in range(len(P)):
        ...
        bufB = h.Helper.paramAddArray(7*P[i], B, 0)
        with open('output.dat', 'a') as output:
            print('\n----------------------------------------------------------------', file=output)
